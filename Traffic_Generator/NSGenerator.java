...
/**
 * Class to generate traffic going from North to South & vice versa
 * for part 2 of the assignment
 */

public class NSGenerator implements Runnable {
    /**
     * Instance Variables
     */
    private Grid grid;
     ...
     * @param g - Grid
     */
    public NSGenerator(Grid g, GridBuilder gb) {
        this.grid = g;
        this.gridBuilder = gb;
        isActive = true;
    ...
    public Car createCar() {
        // Choose the direction and create the new car
        String direction = directionChooser();
        Car newCar = new Car(grid, direction, gridBuilder);
        carArrayList.add(newCar);

        // Initialise co-ordinate and marker
        int xPos = 0;
        int yPos = 0;
        String marker = "";

        // Generate a random speed between max speed and min speed
        int speed = maxSpeed + (int) (Math.random() * ((minSpeed - maxSpeed) + 1));

        // Maximum possible x co-ordinate for the grid
        int xMax = grid.getCols() - 1;

        // Get the initial co-ordinates and marker
        // It is assumed that all lanes will be used
        if (direction.equals("N")) {
            xPos = (int) (Math.random() * (NLanes));
            yPos = 0;
            marker = "o|";
        } else if (direction.equals("S")) {
            xPos = NLanes + (int) (Math.random() * ((xMax - NLanes) + 1));
            yPos = grid.getRows() - 1;
            marker = "+|";
        }
